"""Contains:
    - ArduinoControllerMainFrame class which implements the methods used by
      the GUI defined in ArduinoControllerGUI.py inside mainFrame class
    - definition of the ArduinoControllerApp which uses the GUI to run
...
helpMsg = ( "Control servo v 1.0\n"
            ...
            "then relays the chosen value to another Arduino over an I2C bus. "
            "Transfer may be paused at any time by pressing the Pause button. "
            " Pressing it again will resume the communication from the last known point.\n"
            "\n"
            "Copyright by Artur Lidtke & Aleksander Lidtke, 2015")

import os, sys, serial, glob
import wx
import ArduinoControllerGUI
...
class servoControllermainFrame( ArduinoControllerGUI.mainFrame ):
    ...
    def __init__( self, parent ):
        ...
        ArduinoControllerGUI.mainFrame.__init__( self, parent )
    ...
    def updateState ( self ):
        ...
        if self.portOpen and not self.pause:
            ...
            if self.checkConnection():
                # send the message
                self.arduinoSerialConnection.write(self.inputStartChar + 'sX'
